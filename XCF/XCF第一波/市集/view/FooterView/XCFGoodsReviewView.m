...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.backgroundColor=XCFDishViewBackgroundColor;
        ...
        [_lookReview mas_makeConstraints:^(MASConstraintMaker *make) {
            make.bottom.equalTo(self.mas_bottom).offset(-113);
            make.centerX.equalTo(self);
            make.size.mas_equalTo(CGSizeMake(100, 40));
        }];
        
        _fooView=[[UIView alloc]init];
        _fooView.backgroundColor=XCFGlobalBackgroundColor;
        [self addSubview:_fooView];
        [_fooView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        _pushLabel.text=@"继续拖动，查看图文详情";
        _pushLabel.font=[UIFont systemFontOfSize:14];
        [_fooView addSubview:_pushLabel];
        [_pushLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.equalTo(self.fooView.mas_centerX);
             make.centerY.equalTo(self.fooView.mas_centerY);
